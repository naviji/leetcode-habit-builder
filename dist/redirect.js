let problemUrl = "";
export async function setRedirectRule(redirectUrl) {
    problemUrl = redirectUrl;
    console.log("setRedirectRule", redirectUrl);
...
export async function unsetRedirectRule() {
    ...
    if (urlTracker) {
        ...
        for (const [tabId, url] of Object.entries(urlTracker)) {
            ...
            chrome.tabs.update(Number(tabId), { url: url });
        }
        await chrome.storage.local.remove("urlTracker");
    }
    console.log("urlTracker", urlTracker);
}
const handleBeforeNavigation = async (details) => {
...
const handleOnCommitted = async (details) => {
    if (details.frameId === 0) {
